...
function renderTextNode(module, element, component) {
    ...
    while (childNode !== null) {
        ...
        const segments = utils.parseExpression(childNode.nodeValue);
        ...
        for (let textSegment = 0; textSegment < segments.length; textSegment ++) {
            ...
            if (!segment.isExpression) {
                ...
                const textNode = document.createTextNode(utils.evaluate(model, segment.value));
                element.insertBefore(textNode, childNode);
                component.addBinding(textNode, () => {
                    textNode.nodeValue = utils.evaluate(model, segment.value);
                });
            }
        }
...
function render(module, element, component) {
    ...
    if (componentFactory) {
        ...
        const childComponent = componentFactory(module, childModel);
    ...
    for (let i = 0; i < attributes.length; i ++) {
        ...
        if (decoratorFactory) {
            ...
            component.addBinding(attribute, () => {
                attribute.value = utils.getSegmentsValue(model, segments);
            });
        }
    }
...
class Module {
     ...
     * @param {*} rootModel 
     */
    bootstrap(rootElement, rootModel) {
